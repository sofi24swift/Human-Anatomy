...
export default function LibraryTab() {
  ...
  return (
    <div className="bg-white/90 p-6 rounded-3xl border-4 border-indigo-200 shadow-xl max-w-4xl mx-auto">
      <div className="flex items-center gap-2 mb-4">
        <Book className="w-8 h-8 text-indigo-600 stroke-[1.5] animate-pulse" />
        <div>
          <h2 className="text-xl font-extrabold text-indigo-900">წყაროები და სამეცნიერო ლიტერატურა</h2>
          <p className="text-xs text-indigo-600">საიდან მოდის აპლიკაციაში მოცემული სამედიცინო ფაქტები?</p>
        </div>
      </div>

      <div className="bg-indigo-50 p-4 rounded-2xl border border-indigo-150 mb-6 text-xs text-indigo-950 leading-relaxed font-semibold">
        🛡️ <span className="text-indigo-800 font-extrabold">აკადემიური სიზუსტე:</span> ჩვენი აპლიკაციის თითოეული სამეცნიერო ფაქტი, დაწყებული სახსრების ლათინური სახელწოდებებიდან კანის შეხორცების ეტაპებამდე, ეყრდნობა ნამდვილ სამედიცინო სახელმძღვანელოებსა და კლინიკურ სახელმძღვანელოებს. აქ მოცემულია ძირითადი წყაროები:
      </div>
      ...
      <div className="grid grid-cols-1 gap-5">
        {sourcesList.map((source, index) => (
          <div
            key={index}
            className="border-2 border-slate-100 rounded-2.5xl p-5 hover:border-indigo-250 transition-colors duration-300 bg-slate-50/50 hover:bg-white flex flex-col md:flex-row gap-4 items-start"
          >
            ...
            <div className="flex-1">
              <div className="flex flex-wrap items-baseline gap-x-2">
                <h3 className="font-extrabold text-sm text-indigo-900">{source.title}</h3>
                <span className="text-[10px] text-slate-400 font-mono italic">{source.originalName}</span>
              </div>
              <span className="text-[10px] text-slate-500 font-bold block mt-0.5 mb-2">ავტორი: {source.author}</span>
              <p className="text-xs text-slate-600 leading-relaxed mb-3">{source.description}</p>
              
              <div className="bg-white px-3 py-2 rounded-xl border border-indigo-100 text-[11px] text-indigo-950 font-medium">
      ...
      <div className="mt-6 pt-5 border-t border-slate-150 flex flex-col md:flex-row justify-between items-center gap-3">
        ...
        <a
          ...
          className="text-xs font-black text-indigo-650 flex items-center gap-1 hover:underline"
        >
